...
/*
//only receive용으로 적합
void CCommThread::WatchThreadStart()
{
	DWORD dwThreadID=0;
...
void CCommThread::receive()
{
	if(m_bConnected == TRUE)	OnReceiveData();
}
...
void CCommThread::ClosePort()
{
	...
	if(m_hThreadWatchComm)	SetEvent(m_hComm);
...
void CCommThread::SetCallFuntion(void(*Fun)(void))
{


}
void CCommThread::OnReceiveData()
{

}

void CCommThread::ClearComErr()
{
	DWORD	 dwErrorFlags;
	COMSTAT comstat;
	ClearCommError( m_hComm, &dwErrorFlags, &comstat);
...
DWORD	ThreadWatchComm(CCommThread* pComm)
{
  ...
  //pComm ->m_bConnected
   while (pComm )//포트가 연결되면 무한 루프에 들어감
   {
 		dwEvent = 0;
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
        {
			...
			do
			{
				...
				{
					//AfxMessageBox("Serial Commport buff full"); //큐버퍼의 크기를 초과하면 경고 메시지 보냄
				}

				if(pComm ->m_bConnected==FALSE) break;
			}while(dwRead && pComm);

			if(pComm)	pComm->receive();
			//::PostMessage(hCommWnd, WM_COMM_READ, 0, 0 );//CSerialComDlg로 데이터가 들어왔다는 메시지를 보냄
		}
  ...
  CloseHandle( os.hEvent);
   ...
   //--> 쓰레드 종료가 되겠죠?
  if(pComm)   pComm->m_hThreadWatchComm = NULL;

  TRACE("COMMTHREAD EXITED \n");
   return TRUE;
